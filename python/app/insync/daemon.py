...
import sys, os, optparse, pwd, grp, signal

class Daemon(object):
	def __init__(self, stdin="/dev/null", stdout="/dev/null", stderr="/dev/null", conf="", workers="", pidfile=None, user=None, group=None):
		options = dict(
			stdin=stdin,
			stdout=stdout,
			stderr=stderr,
	...
	def optionparser(self):
		...
		p.add_option("-w", "--workers", dest="workers", help="max workers (default %default)", default=self.options.workers)
		return p
