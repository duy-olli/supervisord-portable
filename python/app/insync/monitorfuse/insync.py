...
def sendto_queue(line, include_tree, exclude_tree, watch_queue_dict):
	if len(line.strip().split('|')) < 3:
		sys.stdout.write("%s : bad queue %s\n" % (time.strftime("%d/%m/%Y %H:%M:%S",time.localtime()),line))
		sys.stdout.flush()
		return
		...
		if (not fd or currname != minute) and os.path.exists(log_path) and (time.time() - os.stat(log_path).st_mtime) <= 60:
			if fd: 
				...
				while line != '': 
					...
					if line.endswith("\n"):
						if len(line.strip().split('|')) < 3:
							print line, "and", oldline
						sendto_queue(splitted + line, include_tree, exclude_tree, queue_dict)
						splitted = ''
					else:
						splitted += line
