...
import multiprocessing, time, ConfigParser, sys, os, string, hashlib, setproctitle, signal, socket, struct, stat
...
def watch_section_job(sections): 
	...
	sock.bind(monitorfs_socket_file)
	sock.listen(1)
	os.chmod(monitorfs_socket_file, stat.S_IRWXO|stat.S_IRWXU|stat.S_IRWXG|stat.S_IFSOCK)
	...
	while not isexit:
		...
		while not isexit:
			try:
				...
				break
			if not line: break
			sendto_queue(line[:-1], include_tree, exclude_tree, queue_dict)
		conn.close()
	sock.close()
	watchjob.join()

monitorfs_mount_dir = None
monitorfs_socket_file = "/tmp/monitor-socket"
...
def read_config(config_path):
	...
	if not monitorfs_mount_dir:
		print "Section monitorfs or samba-dir items don't exists !!"
		return None, None

	if os.path.exists(monitorfs_socket_file):
		try:
			os.unlink(monitorfs_socket_file)
		except OSError:
			print "Cannot delete %s file" % monitorfs_socket_file
			return None, None
...
daemon = Daemon(
	...
	group="root",
        stdin="/dev/null",
        stdout="/var/log/insync/insync2.log",
        stderr="/var/log/insync/insync2.err",
        pidfile="/var/log/insync/insync2.pid",
	conf="./insync.conf",
...
if __name__ == "__main__":
	...
	if daemon.service():
		...
		os.umask(022)
		os.chdir('/opt/python/appnew/insync')
		sys.path.append('/opt/python/appnew/insync')
