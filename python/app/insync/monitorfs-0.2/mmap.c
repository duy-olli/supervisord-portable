...
/**
 * monitorfs_readpage - do something.
 * @file: is something.
 * @page: is something.
 *
 ...
 * Returns some result.
 */
static int monitorfs_readpage(struct file *file, struct page *page)
{
	...
	if (IS_ERR(lower_page)) {
		...
		lower_page = NULL;
		printk(KERN_ERR "monitorfs: Error reading from page cache.\n");
		goto out;
	}
	...
	if (!page_data) {
		err = -ENOMEM;
		printk(KERN_ERR "monitorfs: Error mapping page.\n");
		goto out;
	}
	...
	if (!lower_page_data) {
		err = -ENOMEM;
		printk(KERN_ERR "monitorfs: Error mapping lower page.\n");
		goto out;
	}
