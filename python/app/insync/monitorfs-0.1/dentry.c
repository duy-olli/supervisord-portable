 ...
 */
STATIC int
monitorfs_d_revalidate(struct dentry *dentry, struct nameidata *nd)
{
...
STATIC int
monitorfs_d_hash(struct dentry *dentry, qstr_t *name)
{
	int err = 0;
	struct dentry *lower_dentry;

	print_entry_location();
	lower_dentry = monitorfs_lower_dentry(dentry);	/* CPW: Moved to after print_entry_location */
...
STATIC int
monitorfs_d_compare(struct dentry *dentry, qstr_t *a, qstr_t *b)
{
	int err;
	struct dentry *lower_dentry;
...
int
monitorfs_d_delete(struct dentry *dentry)
{
...
void
monitorfs_d_release(struct dentry *dentry)
{
	struct dentry *lower_dentry;

	print_entry_location();
	fist_print_dentry("monitorfs_d_release IN dentry", dentry);
	...
	fist_print_dentry("monitorfs_d_release IN lower_dentry", lower_dentry);
	if (lower_dentry && lower_dentry->d_inode)
		fist_dprint(6, "monitorfs_d_release: lower_inode->i_count %d, i_num %lu.\n",
                            atomic_read(&lower_dentry->d_inode->i_count),
                            lower_dentry->d_inode->i_ino);

	/* free private data (monitorfs_dentry_info) here */
...
/*
 * we don't really need monitorfs_d_iput, because dentry_iput will call iput() if
 * monitorfs_d_iput is not defined. We left this implemented for ease of
 * tracing/debugging.
 */
void
monitorfs_d_iput(struct dentry *dentry, inode_t *inode)
{
	print_entry_location();
	iput(inode);
	print_exit_location();
