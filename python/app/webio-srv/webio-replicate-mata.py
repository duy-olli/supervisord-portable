...
if __name__ == "__main__":
	if daemon.service():
		...
		host_list = map(lambda x: x.strip(), open("replicate-mata.conf", "r").readlines())
		...
		port = 20203 if not daemon.options.port.isdigit() else int(daemon.options.port)
